...
class PositionState:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PositionState":
        ...
        decimal_fields = ["qty", "entry_price", "exchange_qty", "exchange_entry_price", 
                         "mark_price", "pnl", "pnl_percent"]
...
class PositionStateManager:
    ...
    def sync_with_exchange(self) -> bool:
        ...
        try:
            ...
            if self.position.entry_price != 0:
                pnl_percent = (self.position.pnl / (self.position.entry_price * self.position.qty)) * 100
    ...
    def _check_discrepancies(self, exchange_position: Dict[str, Any]) -> None:
        ...
        if exchange_avg_price > 0:
            price_diff_percent = abs(self.position.entry_price - exchange_avg_price) / exchange_avg_price * 100
    ...
    def validate_position(self) -> tuple[bool, str]:
        ...
        if self.position.discrepancy_detected:
            ...
            if "Qty mismatch" in self.position.discrepancy_details and \
               self.position.exchange_qty is not None and self.position.exchange_qty == 0:

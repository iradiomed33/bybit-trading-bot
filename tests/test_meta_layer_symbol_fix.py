...
class TestMetaLayerSymbolFix:
    ...
    def sample_data_with_anomaly(self):
        ...
        # Создаем данные с экстремальной тенью (аномалия)
        data = pd.DataFrame({
            "open": [100.0] * 100,
            "high": [102.0] * 99 + [130.0],  # Последняя свеча с экстремальной тенью
            "low": [98.0] * 100,
            "close": [100.0] * 99 + [100.5],  # Малое тело на последней свече
            "volume": [1000] * 100
        })
        return data
    ...
    def test_no_trade_zones_integration_with_details(self, feature_pipeline):
        ...
        """
        # Создаем данные с несколькими типами аномалий
        data = pd.DataFrame({
            "open": [100.0] * 60 + [100.0],
            "high": [101.0] * 60 + [125.0],  # Экстремальная тень
            "low": [99.0] * 60 + [99.0],
            "close": [100.0] * 60 + [100.1],
            "volume": [1000] * 60 + [50]     # Низкий объем
        })

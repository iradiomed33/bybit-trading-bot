...
class AdvancedRiskLimits:
    ...
    def evaluate(self, state: Dict) -> Tuple[RiskDecision, Dict]:
        ...
        if violations:
            ...
            if critical_violations:
                ...
                reason = f"CRITICAL violations: {', '.join(v.check_name for v in critical_violations)}"
            ...
            logger.debug(f"Risk evaluation: ALLOW")
    ...
    def _check_notional(self, state: Dict) -> RiskCheckResult:
        ...
        overage_percent = (total_notional - max_notional) / max_notional if total_notional > max_notional else Decimal(0)
    ...
    def _check_drawdown(self, state: Dict) -> RiskCheckResult:
        ...
        if self.session_start_equity > 0:
            ...
            drawdown_percent = (drawdown_amount / self.max_equity * 100) if self.max_equity > 0 else Decimal(0)
        else:
        ...
        logger.debug(
            f"Drawdown check: {drawdown_percent:.2f}% <= {max_drawdown:.2f}%? {passed}"
        )

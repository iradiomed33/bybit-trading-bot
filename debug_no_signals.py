    ...
    try:
        ...
        for strategy in bot.meta_layer.strategies:
            ...
            if strategy.name == "TrendPullback":
                ...
                logger.info(f"   - Breakout specific conditions would be checked here")
                
            elif strategy.name == "MeanReversion":
                vol_regime = latest.get("vol_regime", 0)
                logger.info(f"   - Need vol_regime == -1 (LOW): {vol_regime == -1} (actual: {vol_regime})")
            ...
            try:
                ...
                if signal:
                    ...
                    logger.info(f"   - Signal: None (conditions not met)")

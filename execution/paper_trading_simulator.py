...
class PaperTradingConfig:
    ...
    # Slippage configuration (базовая поддержка через bps)

    slippage_bps: Decimal = Decimal("2")  # Base slippage in basis points

    slippage_volatility_factor_enabled: bool = True  # Множитель волатильности

    slippage_volume_factor_enabled: bool = True  # Множитель объема

    # Legacy support (для обратной совместимости)

    slippage_buy: Optional[Decimal] = None  # Deprecated - use slippage_bps

    slippage_sell: Optional[Decimal] = None  # Deprecated - use slippage_bps

    slippage_volatility_factor: Decimal = Decimal("1.0")  # Legacy множитель волатильности
...
class PaperTradingSimulator:
    ...
    def __init__(self, config: PaperTradingConfig = None):
        ...
        # Legacy support: если используются старые параметры slippage_buy/sell, конвертируем их

        if self.config.slippage_buy is not None:

            # Преобразуем slippage_buy (0.0001) в slippage_bps (1 bps)

            self.config.slippage_bps = self.config.slippage_buy * Decimal("10000")
        ...
        logger.info(
            ...
            "PaperTradingSimulator initialized: "
            ...
            f"slippage={float(self.config.slippage_bps):.1f} bps"
        ...
        if side == "Buy":
            ...
            if self.cash < estimated_cost:
                ...
                return (
                    ...
                    f"Insufficient cash: need ${float(estimated_cost):.2f}, have ${float(self.cash):.2f}",
    ...
    def _fill_order(
        ...
        filled_price = self._calculate_filled_price(current_price, order.side, is_market=is_market)

        # Вычислить комиссию

        commission_rate = (

            self.config.taker_commission if is_market else self.config.maker_commission
        ...
        if order.side == "Buy":
            ...
            if self.cash < total_cost:
                ...
                return False, "Insufficient cash for order fill"
    ...
    def _calculate_filled_price(
        ...
        # Вычислить slippage из BPS

        # Преобразуем BPS в десятичное значение: 2 bps = 0.0002

        slippage_ratio = self.config.slippage_bps / Decimal("10000")
        ...
        if self.config.use_random_slippage:
            ...
            slippage = slippage_ratio * Decimal(str(0.5 + self._rng.random()))

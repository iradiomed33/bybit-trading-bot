...
class TradeMetricsCalculator:
    ...
    def calculate(
        ...
        if metrics.total_trades > 0:
            ...
            metrics.sl_hit_rate = (Decimal(metrics.trades_hit_sl) / Decimal(metrics.total_trades)) * 100
            metrics.tp_hit_rate = (Decimal(metrics.trades_hit_tp) / Decimal(metrics.total_trades)) * 100
        ...
        if metrics.gross_profit != 0:
            metrics.commission_as_percent_of_pnl = (
                (metrics.total_commission_paid / abs(metrics.gross_profit)) * 100
            )
    ...
    def format_metrics(metrics: TradeMetrics) -> Dict[str, str]:
        ...
        return {
            ...
            "Profit Factor": f"{float(metrics.profit_factor):.2f}" if metrics.profit_factor != Decimal("inf") else "∞",
            "---": "---",
            ...
            "Recovery Factor": f"{float(metrics.recovery_factor):.2f}" if metrics.recovery_factor > 0 else "N/A",
            "---": "---",

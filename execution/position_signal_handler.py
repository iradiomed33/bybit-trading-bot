...
class PositionSignalHandler:
    ...
    def __init__(self, config: SignalActionConfig = None):
        ...
        logger.info(
            f"PositionSignalHandler initialized: "
    ...
    def _get_action_for_same_direction(
        ...
        action = (
            self.config.long_signal_action
            if signal_is_long
            else self.config.short_signal_action
        )
    ...
    def validate_add_action(
        ...
        if self.config.require_higher_confidence:
            ...
            if confidence < self.config.min_confidence_for_action:
                return False, f"Confidence too low: {confidence} < {self.config.min_confidence_for_action}"
        ...
        if pyramid_level >= self.config.max_pyramid_levels:
            return False, f"Max pyramid levels reached: {pyramid_level} >= {self.config.max_pyramid_levels}"

        # Проверка 3: Увеличение qty не превышает лимит?
        current_qty = Decimal(str(current_position.get("qty", 0)))
        max_qty_increase = current_qty * Decimal(str(self.config.max_position_qty_increase_percent / 100))
        ...
        if new_notional > self.config.max_total_exposure_usd:
            return False, f"Total exposure too high: {new_notional} > {self.config.max_total_exposure_usd}"

        logger.info(
            f"ADD validation passed: "
...
class PositionManager:
    ...
    def close_position(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        logger.info(
            f"Position CLOSED: {symbol} {position['side']} qty={position['qty']}"
        )
